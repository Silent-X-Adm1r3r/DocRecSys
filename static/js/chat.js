...
function startChat() {
    ...
    addBotBubble(
        ...
        "Describe your symptoms in plain language and I'll help you " +
        "understand possible conditions and recommend the right specialist.\n\n" +
...
async function sendMessage() {
    ...
    try {
        ...
        removeLoading(loader);
        addBotBubble("⚠️ Something went wrong. Please try again.");
        console.error(err);
    } finally {
...
function renderEmergency(data) {
    ...
    card.innerHTML = `
        <div class="emer-title">🚨 EMERGENCY ALERT</div>
...
function renderResult(data) {
        ...
        html += `
            <div class="condition-row">
                ...
                <div class="confidence-bar">
                    ...
                    <span class="confidence-pct">${c.confidence_pct}</span>
                </div>
            </div>
    ...
    if (data.doctor_recommendation) {
        ...
        for (const doc of dr.doctors) {
            const initials = doc.name.split(" ").filter(w => w[0] === w[0].toUpperCase()).map(w => w[0]).join("").slice(0, 2);
            html += `
                <div class="doctor-card">
                    <div class="doctor-avatar">${initials}</div>
                    <div class="doctor-info">
                        <div class="doc-name">${esc(doc.name)}</div>
                        <div class="doc-meta">${esc(doc.hospital)} · ${esc(doc.city)} · ${esc(doc.contact)}</div>
                    </div>
                </div>`;
        }
        html += `</div></div>`;
